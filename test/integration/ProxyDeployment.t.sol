...
contract ProxyDeploymentTest is Test {
    function testProxyDeploymentWiring() public {
        ...
        bytes memory eventManagerInitData = abi.encodeCall(EventManager.initialize, (owner, address(0), address(oracleAdapter)));
        ...
        bytes memory fundingManagerInitData = abi.encodeCall(FundingManager.initialize, (owner, address(0), address(eventManager)));
        ...
        bytes memory orderBookManagerInitData =
            abi.encodeCall(OrderBookManager.initialize, (owner, address(eventManager), address(fundingManager), address(feeVaultManager)));
